...
export default function FactoryStock() {
  ...
  const filteredModels = useMemo(() => {
    ...
    // Sort: active first, then by power ascending (small to large); discontinued at the end
          ...
          <CardHeader className="shrink-0 flex flex-col sm:flex-row sm:items-start justify-between gap-3 space-y-0 py-4 px-4 sm:px-6 bg-gradient-to-r from-slate-100 via-primary/10 to-slate-100 dark:from-slate-800/80 dark:via-primary/15 dark:to-slate-800/80 border-b border-border rounded-t-lg">
            ...
            <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto sm:shrink-0 min-w-0">
              <Button
                ...
                onClick={() => {
                  downloadStockPdf(modelsByCategory, modelStatistics, false, true);
                  toast.success('Stock report downloaded and opened for print');
                }}
              >
        ...
        {modelsByCategory.map(({ key, title, models: categoryModels, config }) => {
          ...
          return categoryModels.length > 0 ? (
            <div key={key} className="space-y-4">
              <div className={cn('flex items-center gap-3 rounded-xl px-4 py-3 shadow-sm border border-white/10', config.headerClass)}>
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-white/25 dark:bg-white/20">
                  <Icon className="h-5 w-5 text-white" />
                </div>
                <div className="min-w-0">
                  <h2 className="text-base font-bold tracking-tight text-white">{title}</h2>
                  <p className="text-xs font-medium text-white/90">{categoryModels.length} model{categoryModels.length !== 1 ? 's' : ''}</p>
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {categoryModels.map((model) => {
                  ...
                  const rows: { icon: React.ReactNode; label: string; value: number }[] = [
                    { icon: <Package className="h-3.5 w-3.5" />, label: 'Total Registered', value: stats.totalRegistered },
                    { icon: <Warehouse className="h-3.5 w-3.5" />, label: 'In Factory', value: stats.inFactory },
                    { icon: <Truck className="h-3.5 w-3.5" />, label: 'To Dealer', value: stats.toDealer },
                    { icon: <Users className="h-3.5 w-3.5" />, label: 'To Sub-Dealer', value: stats.toSubDealer },
                    { icon: <ShoppingCart className="h-3.5 w-3.5" />, label: 'Not Yet Sold', value: stats.notYetSold },
                  ];

                  return (
                    <Card
                      key={model._id}
                      className="overflow-hidden border border-slate-200/90 dark:border-slate-700/90 bg-card shadow-sm hover:shadow-md hover:border-slate-300/80 dark:hover:border-slate-600/80 transition-all duration-200"
                    >
                      <CardHeader className={cn('py-3 px-4 border-b border-white/20 dark:border-white/10', config.cardHeaderClass)}>
                        <CardTitle className="flex items-center justify-between gap-2 text-sm font-semibold leading-tight">
                          <span className="truncate font-mono text-foreground">{getModelDisplayName(model)}</span>
                          <span className="shrink-0 rounded-full bg-white/25 dark:bg-black/20 px-2.5 py-1 text-xs font-semibold tabular-nums text-foreground">
                      ...
                      <CardContent className="p-0">
                        <div className="divide-y divide-border/60">
                          {rows.map(({ icon, label, value }) => (
                            <div
                              key={label}
                              className="flex items-center justify-between gap-3 px-4 py-2.5 first:pt-3 last:pb-3"
                            >
                        ...
                        {/* Serial Numbers - when model is filtered */}
                        {modelId === model._id && modelSerialNumbers.length > 0 && (
                          <div className="mx-4 mb-4 p-3 rounded-lg bg-muted/50 border border-border/60">
                            <h4 className="text-[11px] font-semibold text-foreground uppercase tracking-wider mb-2">
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5 max-h-28 overflow-y-auto">
                              {modelSerialNumbers.map((serialNumber) => (
                                <Link
                                  ...
                                  className="px-2 py-1 text-[11px] font-mono font-medium rounded border border-border/60 bg-background/80 text-foreground hover:bg-muted transition-colors text-center truncate"
                                >
                                  {serialNumber}
                                </Link>
                              ))}
